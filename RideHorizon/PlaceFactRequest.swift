...
enum PlaceFactError: LocalizedError, Equatable {
    case missingProxyToken
    case invalidResponse
    case httpError(Int)
    case speechServiceUnavailable(code: String)

    var errorDescription: String? {
        switch self {
        case .missingProxyToken:
            return "missing proxy token"
        case .invalidResponse:
            return "invalid proxy response"
        case .httpError(let statusCode):
            return "proxy HTTP \(statusCode)"
        case .speechServiceUnavailable(let code):
            return "Premium voice is temporarily unavailable. [\(code)]"
        }
    }
}
