...
public class PromptOverridesService {
    ...
    public PromptOverridesService(HttpClient httpClient, ObjectMapper objectMapper, RideHorizonProperties properties) {
    ...
    private void requirePromptOverridesHostAllowlistConfigured(String objectUrl) {
        ...
        if (allowedHosts.isEmpty()) {
            throw new IllegalStateException(
                    "prompt override host allowlist is required when RIDEHORIZON_PROMPT_OVERRIDES_ENABLED=true"
            );
        }
        URI uri = parsePromptOverridesUri(objectUrl);
