...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleInvalidRequest(HttpMessageNotReadableException ex) {
        log.warn("event=request_rejected status=400 reason=invalid_json");
    ...
    @ExceptionHandler(SpeechUpstreamException.class)
    public ResponseEntity<Map<String, String>> handleSpeechUpstream(SpeechUpstreamException ex) {
        log.warn(
                "event=speech_request_upstream_failure status=502 diagnosticCode={} upstreamStatus={}",
                ex.diagnosticCode(),
                ex.upstreamStatus()
        );
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                .body(Map.of(
                        "error", SpeechUpstreamException.RIDER_SAFE_MESSAGE,
                        "code", ex.diagnosticCode()
                ));
    }
