...
3. Run `backlog instructions overview`, then inspect active work with `backlog task list --plain` and `backlog task view RH-XXX --plain`.
...
- Keep current project control in `AGENTS.md`, `PROJECT.md`, `MILESTONES.md`, `backlog.config.yml` and the CLI-managed `backlog/` records.
