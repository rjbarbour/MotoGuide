...
- The High-priority architecture programme uses the tighter default budget in `../architecture/plans/ARCHITECTURE_REFACTOR_PLAN.md`: focused evidence within each batch; complete iOS suites only at baseline, ride-orchestration and final announcement gates; unsigned Release builds only at baseline and final; one proxy baseline; and one final physical iPhone build/install. Do not invoke `test-changed` merely to repeat an already recorded phase gate. `RH-071 — Architecture CI suppression` owns the narrow GitHub iOS suppression rule; standard skip tokens are prohibited and risk-triggered exceptions must be recorded.
- For the three architecture batch pull requests only, the GitHub iOS job may be suppressed after the owning batch has recorded its local iOS gate. The explicit marker contract is a same-repository pull-request head branch matching `codex/rh-013.36-<slug>--local-ios-evidence`, `codex/rh-013.37-<slug>--local-ios-evidence` or `codex/rh-013.38-<slug>--local-ios-evidence`, with a valid base/head comparison. The branch suffix remains through merge and the remote branch is deleted only afterwards. The selector keeps local `ios=true`, emits `github_ios=false` only for an exact valid marker, and leaves `proxy` and `proxy_deploy` unchanged. Missing, malformed, mismatched, forked or otherwise invalid markers fail closed to the normal GitHub iOS job; invalid comparisons exit non-zero. No skip-ci token is recognised.
- For the resulting `push` to `refs/heads/main`, the coordinator must make the merge commit's tip message end with the exact non-empty trailer `Local-iOS-Evidence: RH-013.36`, `Local-iOS-Evidence: RH-013.37` or `Local-iOS-Evidence: RH-013.38`, and the compared paths must include the matching `backlog/tasks/rh-013.xx … .md` batch record. The workflow passes `github.ref` and `github.event.head_commit.message` to the selector for this check. Missing, malformed, mismatched or incomplete merge evidence leaves `github_ios=true`; proxy and `proxy_deploy` remain unchanged.
