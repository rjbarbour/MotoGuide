...
- Keep local Xcode output under `/private/tmp/RideHorizonDerivedData`. `./tools/derived-data path` resolves the current branch's isolated cache and automatically removes caches unused for seven days while preserving evidence-bearing subtrees. After final evidence capture, run `./tools/generated-output complete`; it removes disposable task DerivedData and output, prunes dependency caches unused for 30 days, and retains recent caches plus release evidence. Gradle and privacy-site npm entry points refresh dependency-use markers; use `./tools/generated-output mark-dependency <path>` after other direct dependency-cache use. The first prune initialises a marker on any existing unmarked cache, giving it one 30-day grace period. Use `./tools/generated-output clean-dependencies` only for explicit disk-pressure recovery. Do not create `DerivedData*` directories in the repository root.
