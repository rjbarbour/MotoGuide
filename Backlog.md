...
- **Reliability acceptance evidence:** Focused tests prove recovery after transient network and gateway failures, no retry for permanent/capacity failures, cancellation during backoff, timeout values longer than the proxy upstream deadline, and suppression of superseded Premium Voice fallback. The full iOS unit suite, physical-device compile check, proxy test suite and live health/fact/speech smoke paths must still pass. Fly deployment must preserve the current endpoint and set `min_machines_running = 1` before the increment is reported complete.
